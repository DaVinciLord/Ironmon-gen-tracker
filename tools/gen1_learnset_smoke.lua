...
local bulbasaurInternal = assert(SpeciesMap.getInternalId(1))
...
local evolutions, moves = DataAdapter.readEvolutionsAndMoves(1)
...
assert(not DataAdapter.isRandomizedLearnset(moves))
assert(DataAdapter.isRandomizedLearnset({ { id = 33, level = 1 }, { id = 45, level = 1 } }))
