...
local party = PokemonDataReader.readPartyPokemon(base, function(internalId)
...
assert(party.status == PokemonDataReader.Status.SLEEP and party.sleep_turns == 3)
...
local enemy = PokemonDataReader.readBattlePokemon(base, function() return 25 end)
assert(enemy.level == 30 and enemy.status == PokemonDataReader.Status.BURN)
